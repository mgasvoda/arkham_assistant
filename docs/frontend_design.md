...
- **Features:**
  - Card list with quantity controls
  - Sort by: cost, type, function, class
  - Visual curve display (resource cost distribution)
  - Class/archetype ratio summary
  - Quick-add from search results

### 2. Deck Search View
- **Purpose:** Browse and filter card database
- **Features:**
  - Text search (name, traits, card text)
  - Filters: class, type, cost range, set/expansion
  - "Owned sets only" toggle
  - Click card → detail pane
